...
$sql_leaders = 'SELECT `leaders`.name,components.name as compname, .leaders.date_from,`leaders`.dateto, 
`structures`.name as strname,time_lines.from,time_lines.to FROM `leaders` INNER JOIN 
`components` ON `leaders`.`componentId` = `components`.`id` INNER JOIN structures
 on components.structureId = structures.id INNER JOIN `time_lines`
 ON `structures`.`timeLineId`=`time_lines`.`id` WHERE `leaders`.`componentId`=' . $_GET['compid'].'';
?>
<!DOCTYPE html>
<html lang="en">
                        ...
                        <div class="card">
                            <div class="card-header alert-light d-flex">
                                <h5 class="card-title col-10" id="cardtitle">
                                    <?php
                                    $result_timeline = $conn->query($sql_get_component_and_str_and_timeline);
                                    if ($result_timeline->num_rows > 0) {
                                        $row = $result_timeline->fetch_assoc();
                                        echo 'Abayobozi Bayoboye ' . $row['strname'] . ' ya ' . $row['compname'] . ' kuva ' . $row['from'] . ' kugera ' . $row['to'];
                                    }
